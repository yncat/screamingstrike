...
class CollectionStorage(object):
    ...
    def initialize(self, screamNum, file=""):
        ...
        # end for
        if len(nums) < screamNum:
            self.reserve(screamNum - len(nums))
...
class CollectionCounter(object):
    ...
    def increment(self, number):
        ...
        if self.storage is None:
            return
        if self.storage.increment(number) is True:
            self.unlock(number)
...
class CollectionDialog(object):
    ...
    def run(self, appMain):
        ...
        while(True):
            ...
            if appMain.keyPressed(window.K_RETURN) and appMain.collectionStorage.isUnlocked(self.index):
                self.play(self.index)
            if appMain.keyPressing(window.K_UP) and self.pitchTimer.elapsed >= 50 and self.pitch != enemy.SCREAM_PITCH_HIGH:
                self.changePitch(self.pitch + 1)
            if appMain.keyPressing(window.K_DOWN) and self.pitchTimer.elapsed >= 50 and self.pitch != enemy.SCREAM_PITCH_LOW:
                self.changePitch(self.pitch - 1)
        # end while
    ...
    def searchUnlocked(self, direction):
        ...
        while True:
            i += direction
            if i == -1 or i == self.appMain.numScreams:
                break
