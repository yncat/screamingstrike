...
class SingletonWindow():
    ...
    def frameUpdate(self):
        ...
        if self.keyPressed(K_LCTRL):
            self.sayStop()
        if self.keyPressing(K_LALT) and self.keyPressed(K_F4):
            self.exit()
    ...
    def frameUpdate(self):
        ...
        if processArrows:
            ...
            if dn:
                self.moveTo(self.cursor + 1)
        # end arrow keys
        if self.wnd.keyPressed(K_HOME) and self.cursor != 0:
            self.moveTo(0)
        if self.wnd.keyPressed(K_END) and self.cursor != len(self.items):
        ...
        if self.wnd.keyPressed(K_PAGEDOWN):
            ...
            if n > 0:
                self.moveTo(self.cursor + n)
        # end pagedown
        if self.wnd.keyPressed(K_SPACE):
